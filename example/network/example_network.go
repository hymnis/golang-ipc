...
func server() {
	config := &ipc.ServerConfig{Network: true}
	...
	go func() {
		for {
			...
			if err == nil {
				if m.MsgType > 0 {
					log.Println("Server receieved: "+string(m.Data)+" - Message type: ", m.MsgType)
				}
			} else {
...
func client() {
	config := &ipc.ClientConfig{Network: true}
	...
	go func() {
		for {
			...
			if err != nil {
				// An error is only returned if the receieved channel has been closed,
			...
			if m.MsgType > 0 { // all message types above 0 have been receieved over the connection
				log.Println("Client receieved: "+string(m.Data)+" - Message type: ", m.MsgType)
			}
		}
